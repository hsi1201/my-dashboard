...
/* 🌟 금액 숫자(Value) 반응형 크기 조절 (세로모드 잘림 방지) */
...
<div style="margin-top: -15px; margin-bottom: 10px;">
    <h2 style="margin-bottom: 0px; padding-bottom: 5px; font-size: 1.8rem;">📊 글로벌 마켓 대시보드 (v1.0.47 스마트 캘린더 자동화)</h2>
</div>
""", unsafe_allow_html=True)
...
# ---------------------------------------------------------
# 🌟 [데이터 파서] 엑셀 처리 함수 (오류 방지용 튼튼한 파싱)
# ---------------------------------------------------------
def clean_currency_str(val):
    """₩, \, +, , 등 계산에 방해되는 문자열을 모두 안전하게 제거"""
...
def parse_portfolio_excel(file):
    ...
    for idx, row in df_inv.iterrows():
        ...
        if val_clean == 'IRP-장기': 
            current_account = 'IRP - 장기'
            continue
        elif val_clean == 'ISA-중기': 
            current_account = 'ISA - 중기'
            continue
        elif val_clean == '국내주식-단기': 
            current_account = '국내주식 - 단기'
            continue
        elif val_clean == '해외주식-단기': 
            current_account = '해외주식 - 단기'
            continue
        elif val_clean == '비상금': 
            current_account = '비상금'
            continue
        elif val_clean == '가상화폐': 
            current_account = '가상화폐'
            continue
        elif val_clean == '부동산': 
            current_account = '부동산'
            continue
        ...
        profit_loss = pd.to_numeric(row.get('손익', 0), errors='coerce') 
        ...
        profit_loss = 0 if pd.isna(profit_loss) else profit_loss
    ...
    total_realized = sum([float(clean_currency_str(account_summaries[acc].get('realized', '0'))) for acc in account_summaries])
    total_invested = sum([float(clean_currency_str(account_summaries[acc].get('buy', '0'))) for acc in account_summaries])
    total_cash = sum([float(clean_currency_str(account_summaries[acc].get('cash_amt', '0'))) for acc in account_summaries])
...
# ---------------------------------------------------------
# 🌟 [자동 로드 엔진 & 수동 업로드 상태 관리]
# ---------------------------------------------------------
LOCAL_EXCEL_PATH = "자산투자관리.xlsx"

def get_file_mod_time():
    return os.path.getmtime(LOCAL_EXCEL_PATH) if os.path.exists(LOCAL_EXCEL_PATH) else 0

@st.cache_data(show_spinner=False)
def load_local_excel_data(mod_time):
    if os.path.exists(LOCAL_EXCEL_PATH):
        try:
            t6 = parse_portfolio_excel(LOCAL_EXCEL_PATH)
            t7 = parse_asset_flow_excel(LOCAL_EXCEL_PATH)
            return t6, t7
...
if "local_mod_time" not in st.session_state:
    ...
    st.session_state.tab6_data, st.session_state.tab7_data = load_local_excel_data(current_mod_time)
    st.session_state.last_uploaded_hash = None
elif current_mod_time != st.session_state.local_mod_time:
    st.session_state.local_mod_time = current_mod_time
    st.session_state.tab6_data, st.session_state.tab7_data = load_local_excel_data(current_mod_time)
    st.session_state.last_uploaded_hash = None
    ...
    if uploaded_file is not None:
        ...
        if st.session_state.get('last_uploaded_hash') != file_hash:
            try:
                ...
                st.toast("수동 업로드된 엑셀 파일로 대시보드가 업데이트되었습니다!", icon="✅")
                st.rerun() 
            except Exception as e:
...
def get_market_data():
    ...
    # 🌟 VIX 독립 수집 최우선 실행 (야후 Bulk 오류 원천 차단)
    try:
        vix_df = yf.Ticker('^VIX').history(start='2026-01-01')[['Close']]
        if not vix_df.empty:
            vix_df.columns = ['VIX']
            vix_df.index = pd.to_datetime(vix_df.index).normalize().tz_localize(None)
            vix_df = vix_df[~vix_df.index.duplicated(keep='last')]
            df_list.append(vix_df)
        else:
            raise ValueError
    except:
        try:
            vix_df = fdr.DataReader('FRED:VIXCLS', '2026-01-01')
            if not vix_df.empty:
                vix_df.columns = ['VIX']
                vix_df.index = pd.to_datetime(vix_df.index).normalize().tz_localize(None)
    ...
    yf_tickers = {
        ...
        '^SOX': '필라델피아 반도체', 'GC=F': '금', 'JPYKRW=X': '엔/원 환율',
...
vix_val = latest_data.get('VIX', None)
...
vix_str = "데이터 없음" if pd.isna(vix_val) or vix_val == 0 else f"{vix_val:,.2f}"
vix_chg = "" if pd.isna(vix_val) or vix_val == 0 else changes.get('VIX', '')
...
with tab1:
    ...
    with cal_col:
        # 🌟 [스마트 캘린더] 내장형 매크로 일정 자동 필터링 로직
        today = pd.Timestamp.now(tz='Asia/Seoul').normalize()
        curr_week_start = today - pd.Timedelta(days=today.weekday())
        curr_week_end = curr_week_start + pd.Timedelta(days=6)
        next_week_start = curr_week_end + pd.Timedelta(days=1)
        ...
        for date_str, event in MACRO_EVENTS.items():
            ev_date = pd.to_datetime(date_str).tz_localize('Asia/Seoul')
            ev_str = f"- {ev_date.strftime('%m/%d')} ({weekdays[ev_date.weekday()]}): {event}"
            
            if curr_week_start <= ev_date <= curr_week_end:
                this_week_events.append(ev_str)
            elif next_week_start <= ev_date <= next_week_end:
                next_week_events.append(ev_str)
                
        tw_text = "\n".join(this_week_events) if this_week_events else "- 예정된 주요 일정 없음"
        nw_text = "\n".join(next_week_events) if next_week_events else "- 예정된 주요 일정 없음"
...
with tab2:
    ...
    with chart_cols[1]:
        ...
        if valid_yield_cols:
            ...
            st.altair_chart(yield_chart, width="stretch")
...
with tab3:
    ...
    if valid_sec_rel_cols:
        ...
        st.altair_chart(sec_line_chart, width="stretch")
...
with tab4:
    ...
    if valid_kr_sec_rel_cols:
        ...
        st.altair_chart(kr_sec_line_chart, width="stretch")
...
with tab6:
    ...
    if pwd == "1016":
        ...
        p_cols = st.columns(4)
        p_cols[0].metric("총 자산 (Total Assets)", t6_metrics.get("총자산", "₩ 0"), t6_metrics.get("평가손익", ""))
        p_cols[1].metric("총 매수금액 (Total Invested)", t6_metrics.get("총매수금액", "₩ 0"), "")
        p_cols[2].metric("실현 손익 (Realized Profit)", t6_metrics.get("실현손익", "₩ 0"), "")
        p_cols[3].metric("계좌 내 현금 비중 (Cash Weight)", t6_metrics.get("현금비중", "0.0%"), t6_metrics.get("현금액", "₩ 0"), delta_color="off")
        ...
        if not df_holdings.empty and not df_port_hist.empty:
            ...
            if active_holdings:
                ...
                st.altair_chart(port_line_chart, width="stretch")
            ...
            for acc in df_holdings["계좌 구분"].unique():
                ...
                def color_profit_loss(val):
                    ...
                    if '+' in val_str:
                        ...
                        return 'color: #4C82FF; font-weight: bold;'
                    return ''
                
                cols_to_style = [c for c in ['평가손익', '수익률(%)'] if c in acc_data.columns]
                
                if cols_to_style:
                    try:
                        styled_df = acc_data.style.map(color_profit_loss, subset=cols_to_style)
                    except AttributeError:
                        try:
                            styled_df = acc_data.style.applymap(color_profit_loss, subset=cols_to_style)
                        except:
                            styled_df = acc_data
                else:
                    styled_df = acc_data
...
with tab7:
    ...
    if pwd2 == "1016":
        ...
        m_cols = st.columns(4)
        m_cols[0].metric("총 순자산 (Total Net Asset)", t7_metrics.get("총순자산", "₩ 0"), t7_metrics.get("총순자산_원", ""), delta_color="off")
        m_cols[1].metric("부동산 순자산 (Real Estate)", t7_metrics.get("부동산", "₩ 0"), t7_metrics.get("부동산비중", ""), delta_color="off")
        m_cols[2].metric("금융 순자산 (Financial Asset)", t7_metrics.get("금융", "₩ 0"), t7_metrics.get("금융비중", ""), delta_color="off")
        m_cols[3].metric("월 여유금 (Monthly Spare Cash)", t7_metrics.get("여유금", "₩ 0"), t7_metrics.get("여유금비중", ""), delta_color="normal")
        ...
        with col1:
            ...
            if not df_t_pie.empty:
                draw_pie_chart(df_t_pie, 'category10')
            if not df_t_table.empty:
                st.dataframe(df_t_table, width="stretch", hide_index=True, column_config=col_config_asset, height=len(df_t_table)*36 + 43)
            
            st.markdown("##### 📊 금융자산 구성 비중")
            if not df_f_pie.empty:
                draw_pie_chart(df_f_pie, 'set2')
            if not df_f_table.empty:
                st.dataframe(df_f_table, width="stretch", hide_index=True, column_config=col_config_asset, height=len(df_f_table)*36 + 43)
        ...
        with col2:
            ...
            if not df_c_table.empty:
                st.dataframe(df_c_table, width="stretch", hide_index=True, column_config=col_config_asset, height=len(df_c_table)*36 + 43)
