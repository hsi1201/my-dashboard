...
def get_market_data():
    ...
    # 🌟 [엔진 1] 야후 파이낸스 서버 (미국/글로벌 지표 전용 - 코스피, 코스닥, 니케이 제외)
    yf_tickers = {
    ...
    # [엔진 1-1] CSI 300 완벽 복구 알고리즘 (TIGER 차이나CSI300)
    try:
        try:
    ...
    # 🌟 [엔진 2] 네이버 금융/KRX 서버 (FDR 활용 - 한국/일본 증시 및 환율 통합)
    fdr_tickers = {
        'KS11': '코스피', 
        'KQ11': '코스닥', 
        'N225': '니케이',
...
with st.expander("📌 데이터 소스 및 타 사이트(Investing.com 등) 수치 차이 안내 (클릭하여 열기)"):
    ...
    *   **W양/음 (선물 월물 교체):** 원자재는 매월 만기가 있는 '선물'입니다. 대시보드(Yahoo API)와 타 사이트가 추종하는 기준 계약(최근월물 vs 차근월물)의 롤오버 시점이 다를 경우 일시적으로 가격 갭이 발생할 수 있습니다.
    *   **아시아 지수 안정화:** 주말 야후 서버 누락을 방지하기 위해 한국(코스피/코스닥)과 일본(니케이) 지수는 네이버/KRX 기반의 실시간 데이터로 교체 적용되었습니다.
    *   **CSI 300 (환노출 반영):** 중국 데이터 통신망 오류를 우회하기 위해 국내 상장 추종 ETF의 과거 궤적을 활용하여 차트를 스케일링합니다.
    *   **해외 지수 (15분 지연):** 일부 무료 API 규정상 거래소 원천 데이터 규정에 따라 15~20분 지연(Delay) 송출될 수 있습니다.
    """)
