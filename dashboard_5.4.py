...
st.title("📊 글로벌 자산투자 시황 대시보드 (UI/UX 6.12)")
...
with st.expander("📌 데이터 업데이트 기준 및 시차 안내 (클릭하여 열기)"):
    ...
    - **한국 국채 (한국은행 10년/30년물):** 실시간 호가가 아닌 한국은행의 1일 1회 공식 마감 통계를 사용하므로, 당일 오후 늦게 전일 데이터가 확정 갱신됩니다.
    - **한국 증시 & 환율 (코스피, 코스피 200, 코스닥, 니케이, 원/달러):** 아시아 장 개장 시간 동안 실시간(또는 15분 지연)으로 정상 갱신됩니다.
    """)
